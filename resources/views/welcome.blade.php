...
<head>
    ...
    <!-- Tailwind CSS CDN -->
    <!-- ⚠️ ملحوظة: دي مناسبة للتطوير بس. للـ production الأفضل تستخدم Tailwind عن طريق Vite build حقيقي بدل الـ CDN -->
    ...
    <style>
        ...
        .glow-blue { box-shadow: 0 0 40px -10px rgba(37, 99, 235, 0.2); }
        .glow-green { box-shadow: 0 0 40px -10px rgba(16, 185, 129, 0.2); }
    </style>
</head>
<body class="antialiased text-slate-800 selection:bg-brand-blue selection:text-white">
    ...
    <header class="fixed top-0 z-50 w-full transition-all duration-300 border-b glass-nav border-slate-100/80">
        <div class="flex items-center justify-between px-4 py-3 mx-auto max-w-7xl sm:px-6">
            ...
            <a href="#" class="flex items-center gap-2 sm:gap-3">
                <img src="{{ asset('assets/images/logo-icon-dark.jpg') }}" alt="Logo" class="rounded-full shadow-sm w-9 h-9 sm:w-10 sm:h-10">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="items-center hidden gap-2 sm:flex">
                    @if (\App\Http\Middleware\CheckGuard::guard())
        ...
        <div id="mobile-menu" class="hidden px-6 py-4 space-y-3 bg-white border-t shadow-inner lg:hidden border-slate-100">
            ...
            <div class="flex flex-col gap-2 pt-2">
                {{--
                    ✅ المشكلة الأساسية كانت هنا: @auth بيفحص الـ default guard بس (admin حسب config/auth.php).
                    يعني معلم/طالب/ولي أمر مسجل دخوله، كان بيشوف "تسجيل الدخول" برضو في قائمة الموبايل.
                    استبدلناها بنفس الشرط المستخدم فوق في نسخة الديسكتوب عشان تتفحص كل الـ guards الأربعة.
                --}}
                @if (\App\Http\Middleware\CheckGuard::guard())
    ...
    <script>
        ...
        /*
         * ✅ اتشالوا من هنا: openDemoModal() و closeDemoModal() و copyText()
         * كانوا بيدوروا على عناصر #demo-modal و #modal-box مش موجودين في الصفحة أصلاً،
         * ومفيش أي زرار بينادي عليهم. لو حد ناداهم كانوا هيرموا خطأ JS (Cannot read properties of null).
         * لو محتاجهم فعلاً (مودال ديمو تجريبي)، قولي عشان نضيف الـ HTML بتاعهم مع الفنكشنز.
         */
