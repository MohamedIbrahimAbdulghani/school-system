                                    ...
                                    <table id="datatable" class="table p-0 table-hover table-bordered" data-page-length="10" style="text-align: center">
                                        <thead>
                                            <tr class="alert-success">
                                                ...
                                                <th>{{ trans('fees.amount_Processing') }}</th>
                                                <th>{{ trans('fees.description_Processing') }}</th>
                                        ...
                                        @foreach($processing_fees as $processing_fee)
                                            ...
                                            <td>
                                                <a href="{{ route('processing_fees.edit', $processing_fee->id) }}" class="btn btn-info btn-sm" role="button" aria-pressed="true"><i class="fa fa-edit"></i></a>
                                                <button type="button" class="btn btn-danger btn-sm" data-target="#delete{{$processing_fee->id}}" data-toggle="modal"><i class="fa fa-trash"></i></button>
                                                <input type="hidden" value="{{ $processing_fee->id }}">
                                                {{-- <a class="dropdown-item btn" data-target="#delete{{$processing_fee->id}}" data-toggle="modal"><i style="color: red" class="fa fa-trash"></i>&nbsp;  {{trans('student.Delete')}}  </a> --}}
                                            </td>
                                            </tr>

                                        {{-- Start Modal To Delete students --}}
                                            <div class="modal fade" id="delete{{$processing_fee->id}}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
                                                    ...
                                                    <form action="{{ route('processing_fees.destroy', $processing_fee->id) }}" method="post">
                                                        ...
                                                        <div class="row">
                                                            <div class="col">
                                                                ...
                                                                <input type="text" readonly value="{{ $processing_fee->student->name }}" class="form-control">
                                                            </div>
                                                        </div>
